...
class DuelingDQN(nn.Module):
    def __init__(self, input_size, num_actions):
        ...
        # Extractor de características común
        ...
        # Flujo de Ventaja (Advantage Stream): Qué tan buena es cada acción
        self.advantage = nn.Sequential(
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, num_actions)
        )
        # Flujo de Valor (Value Stream): Qué tan peligroso/bueno es el estado actual
...
# ═══════════════════════════════════════════════════════════════
#  Trading Environment (Sortino Penalty)
# ═══════════════════════════════════════════════════════════════
class TradingEnv:
    ...
    def step(self, action):
        ...
        if action == 0 and self.position != 0:
            if unrealized_pnl > 0:
                ...
                # PENALIZACIÓN CUADRÁTICA (Sortino Penalty)
                # Normalizamos por 100 USD para que el cuadrado no explote matemáticamente
...
# ═══════════════════════════════════════════════════════════════
def train_dqn_v3():
    print("🚀 Iniciando Motor V3: Dueling DQN + Prioritized Experience Replay (PER) + Sortino Penalty")
    ...
    print(f"🖥️ Dispositivo: {device} | 🧠 State: {input_size} | 🎮 Actions: {num_actions}")
    print(f"🌳 Generando Estructura SumTree Gigante (Puede tardar/consumir RAM)...")
    
    writer = SummaryWriter(log_dir="runs/qlearning_v3")
    
    policy_net = DuelingDQN(input_size, num_actions).to(device)
    target_net = DuelingDQN(input_size, num_actions).to(device)
    target_net.load_state_dict(policy_net.state_dict())
    target_net.eval()
    
    optimizer = optim.AdamW(policy_net.parameters(), lr=LR, weight_decay=1e-4)
    memory = PrioritizedReplayMemory(MEMORY_SIZE)
    criterion = nn.SmoothL1Loss(reduction='none') # 'none' para multiplicar por los IS_weights
    ...
    for episode in range(EPISODES):
        ...
        while not done:
            ...
            if len(memory) > BATCH_SIZE:
                ...
                # Ecuación de Bellman (Double DQN structure para más estabilidad)
                q_values = policy_net(batch_state).gather(1, batch_action)
                with torch.no_grad():
                    # Double DQN: seleccionamos la mejor acción con la Policy Net
                    next_actions = policy_net(batch_next_state).max(1)[1].unsqueeze(1)
                    # Y evaluamos su valor con la Target Net
                ...
                # Pérdida multiplicada por los pesos del Importance Sampling (PER)
                ...
                # Actualizar prioridades en el SumTree
        ...
        print(f"🚀 V3 Episodio {episode+1:03d}/{EPISODES} | Reward: {total_reward:10.2f} | Epsilon: {epsilon:.3f} | Equity: ${env.equity:,.2f}", flush=True)
        
    writer.close()
    print("\n🔄 Exportando Agente AlphaGo V3 a formato ONNX...")
    dummy_input = torch.randn(1, input_size, device=device)
    onnx_path = os.path.join(os.path.dirname(__file__), "models", "brain_qlearning_v3.onnx")
    ...
    print(f"✅ ¡Cerebro V3 Dueling-DQN exportado con éxito a {onnx_path}!")
