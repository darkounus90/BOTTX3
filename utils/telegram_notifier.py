...
from config.settings import TelegramConfig, BotConfig
...
class TelegramNotifier:
    ...
    def __init__(self, logger: BotLogger):
        ...
        self._start_time = datetime.now(ZoneInfo(BotConfig.TIMEZONE))
    ...
    def notify_bot_started(
        ...
        msg = (
            ...
            f"⏱ {datetime.now(ZoneInfo(BotConfig.TIMEZONE)).strftime('%Y-%m-%d %H:%M:%S')}"
        )
        self._send(msg)
    ...
    def notify_bot_stopped(self, reason: str, balance: float, profit: float):
        ...
        uptime = datetime.now(ZoneInfo(BotConfig.TIMEZONE)) - self._start_time
    ...
    def notify_reconnection(self, attempt: int):
        ...
        msg = (
            ...
            f"🔁 Intento: `#{attempt}`\n"
            f"⏱ {datetime.now(ZoneInfo(BotConfig.TIMEZONE)).strftime('%H:%M:%S')}"
        )
        self._send(msg)
    ...
    def notify_daily_summary(
        ...
        msg = (
            ...
            f"⏱ {datetime.now(ZoneInfo(BotConfig.TIMEZONE)).strftime('%Y-%m-%d %H:%M')}"
        )
        self._send(msg)
