...
def sim_brainforge_v6(df, symbol):
    res = BacktestResult("BrainForge V6 Transformer (ONNX)")
    import onnxruntime as ort
    
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'ai_lab', 'models'))
    model_path = os.path.join(base_dir, 'brain_v6_transformer.onnx')
    scaler_path = os.path.join(base_dir, 'scaler_v6.pkl')
    ...
    try:
        ...
        print(f"  [ERROR] Fallo al cargar BrainForge V6 ONNX: {e}")
        return res
    ...
    for i in range(seq_length + 50, len(df)-50):
        ...
        try:
            X_scaled = scaler.transform(X_live)
            X_tensor = X_scaled.astype(np.float32).reshape(1, seq_length, 29)
            
            # Inferencia ONNX
            ort_inputs = {ort_session.get_inputs()[0].name: X_tensor}
        ...
        if prob_win >= 0.40: # Umbral Sniper V6 40%
            signal = "BUY"
            sl = 20.0
            tp = 40.0
            pnl = calculate_pnl(signal, curr['close'], sl, tp, df.iloc[i+1:i+100].to_dict('records'), symbol)
...
def run_backtest(symbol="EURUSD", days=365, z=2.5, adx=45):
    ...
    if symbol == "EURUSD":
        ...
        results = [
            ...
            sim_brainforge_v6(m15.copy(), symbol)
        ]
    elif symbol == "GBPUSD":
