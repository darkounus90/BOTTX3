...
class LondonOpeningBreakoutStrategy(BaseStrategy):
    """
    💂‍♂️ London Opening Range Breakout (European Session)
    =================================================
    Estrategia de fuerza bruta cuantitativa que busca el verdadero
    desplazamiento del dinero inteligente en la apertura del mercado de Londres.
    
    Lógica:
    1. Define el rango de las horas muertas antes de la apertura de Londres (Pre-market).
    2. Espera a que abra Londres (3:00 AM EST).
    ...
    def __init__(self, logger: BotLogger, symbol: str = None):
        ...
        self.logger.info(f"✨ Estrategia Cargada: London Opening Breakout ({self.symbol})")

    def get_name(self) -> str:
        return f"London Breakout Institucional ({self.symbol})"
    ...
    def generate_signal(self) -> dict | None:
        ...
        # Detectar el rango de consolidación pre-Londres (últimas 16 velas = 4 horas)
        lookback_df = df.iloc[-18:-2] # Excluyendo la vela recién cerrada
        
        # Calcular el verdadero techo y piso de esas 4 horas previas
