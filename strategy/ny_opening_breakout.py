...
class NYOpeningBreakoutStrategy(BaseStrategy):
    """
    🌋 NY Opening Range Breakout (Institucional)
    =================================================
    Estrategia de fuerza bruta cuantitativa que busca el verdadero
    desplazamiento del dinero inteligente en la apertura de Wall Street.
    
    Lógica:
    1. Define el rango (High y Low) de la sesión asiática/pre-mercado (ej. 8:00 a 9:30 EST).
    2. Espera a que abra NY (9:30 EST).
    ...
    def __init__(self, logger: BotLogger, symbol: str = None):
        ...
        self.logger.info(f"✨ Estrategia Cargada: NY Opening Breakout ({self.symbol})")

    def get_name(self) -> str:
        return f"NY Breakout Institucional ({self.symbol})"
    ...
    def generate_signal(self) -> dict | None:
        ...
        # 1.5 CONTROL DE HORARIOS ESTRICTOS (Solo operar apertura NY)
